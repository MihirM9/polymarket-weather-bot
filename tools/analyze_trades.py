"""
tools.analyze_trades — Post-hoc trade analysis with actual NWS observations.

Usage:
    python3 -m tools.analyze_trades
"""
import asyncio
import csv
import logging
...
TRADE_LOG = Path(__file__).resolve().parent.parent / "logs" / "trades.csv"
NWS_HEADERS = {
...
def load_trades() -> List[Trade]:
    if not TRADE_LOG.exists():
        logger.error(f"Trade log not found: {TRADE_LOG}")
        sys.exit(1)
    trades: List[Trade] = []
...
async def get_station(session: aiohttp.ClientSession, city: str) -> Optional[str]:
    ...
    if city.lower() in _station_cache:
        return _station_cache[city.lower()]
    coords = match_city(city)
    if not coords:
        return None
    data = await fetch_with_retry(
        session,
        f"https://api.weather.gov/points/{coords[0]:.4f},{coords[1]:.4f}",
        headers=NWS_HEADERS,
        label="nws-points",
    )
    if not data:
    ...
    if not stations_url:
        return None
    sdata = await fetch_with_retry(
        session, stations_url, headers=NWS_HEADERS, label="nws-stations"
    )
    if not sdata or not sdata.get("features"):
        return None
...
async def get_observed_high(
    ...
    local_midnight = datetime(
        obs_date.year, obs_date.month, obs_date.day, 0, 0, tzinfo=local_tz
    )
    ...
    for feat in data.get("features", []):
        ...
        if max_f is None or tf > max_f:
            max_f = tf
    return round(max_f, 1) if max_f is not None else None


def evaluate_trade(t: Trade) -> None:
    if t.actual_temp is None or t.bucket_lo is None or t.bucket_hi is None:
        return
...
async def main() -> None:
    trades = load_trades()
    if not trades:
        print("No filled trades found in log.")
        return
    ...
    async with aiohttp.ClientSession() as session:
        for t in resolved:
            ...
            if not station:
                logger.warning(f"No station for {t.city}")
                continue
