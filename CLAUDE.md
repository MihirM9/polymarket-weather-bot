# CLAUDE.md — Polymarket Weather Trading Bot v3
...
| File | Lines | Purpose |
|------|-------|---------|
| `main.py` | 228 | Async main loop, scan orchestration, graceful shutdown |
| `config.py` | 94 | Loads .env, typed Config singleton |
| `forecast_scanner.py` | 282 | NWS API → Gaussian bucket probabilities, weather regime detection, confidence scoring |
| `ensemble_blender.py` | 281 | NWS + OpenWeatherMap blending, dynamic σ from model disagreement |
| `polymarket_parser.py` | 264 | Gamma API market discovery, bucket parsing (regex), city/date matching |
| `decision_engine.py` | 330 | EV calculation, Kelly criterion, confidence-adaptive tempering, dynamic edge thresholds |
| `position_tracker.py` | 290 | CLOB fill polling, realized vs pending exposure, stale order cancellation |
| `execution.py` | 271 | ClobClient orders, dry-run simulation, CSV logging, Telegram alerts |
...
## Config
All params in `.env`. Key ones: `PRIVATE_KEY`, `BANKROLL`, `KELLY_FRACTION=0.15`, `MIN_EDGE=0.08`, `DAILY_LOSS_CAP=50`, `MODE=dry-run`, `OPENWEATHER_API_KEY`.
...
4. No order book depth check before placing orders
5. No seasonal σ adjustment (spring more volatile than mid-summer)
6. City matching is string-based regex — could mismatch edge cases
